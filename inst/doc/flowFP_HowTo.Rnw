...
data(fs2)
mod1 <-  flowFPModel(fs1, name="CD45/SS Model vs fs1", parameters=c(2,5), nRecursions=7)
mod2 <-  flowFPModel(fs2, name="CD45/SS Model vs fs2", parameters=c(2,5), nRecursions=7)
...
fp <- flowFP (fs1, param=c(2,5), nRecursions=8)
plex <- flowFPPlex()
for (levels in 8:5) {
...
fp1 <- flowFP (fs1, parameters=c(2,5), name="self model: fs1", nRecursions=5)
...
fp2 <- flowFP (fs2, parameters=c(2,5), name="self model: fs2", nRecursions=5)
...
data(plate)
fp <- flowFP (plate, parameters=c(2,5,7), nRecursions=5)
plot (fp, type='plate')
@
\end{center}
